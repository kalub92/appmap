...
- **explore** — you drive Argent with the map as a prior, per rules 2–3; unknown screen → `name_screen`. If `name_screen` refuses because the screen is already `verified` and its stored data is wrong, demote it with `mark {screen_id, status: candidate, reviewer}` and name it again (02 §8).
